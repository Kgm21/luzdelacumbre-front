...
const AdminPage = () => {
  ...
  const sortedBookings = [...bookings].sort((a, b) => new Date(a.checkInDate) - new Date(b.checkInDate));
  const now = new Date();
  const filteredBookings = showPastBookings
    ? sortedBookings
    : sortedBookings.filter(b => new Date(b.checkInDate) >= now);
        ...
        {activeSection === "bookings" && (
          <>
            <Col xs={12} className="mb-3 text-end">
              <Button variant="outline-primary" onClick={() => setShowPastBookings(prev => !prev)}>
                {showPastBookings ? 'Ocultar reservas viejas' : 'Mostrar reservas viejas'}
              </Button>
            </Col>
            <Col xs={12} md={editBookingData ? 8 : 12} className="mb-3">
              <BookingsList bookings={filteredBookings} onEditBooking={handleBookingEditClick} auth={auth} logout={logout} refreshBookings={fetchBookings} />
            </Col>
            <Col xs={12} md={4}>
              {editBookingData ? (
                <>
